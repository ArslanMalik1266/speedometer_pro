...
class SettingsFragment : Fragment() {
    ...
    private fun initViews(view: View) {
        ...
        val support_arrow = view.findViewById<ImageView>(R.id.support_arrow)
        privacyPolicyTv = view.findViewById(R.id.privacy_policy_tv)

        privacyPolicyTv.addPressEffect {
            val url = "http://webscare.com/privacy-policy-speedometer/"
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(url))
            startActivity(intent)
        }

        support_arrow.addPressEffect {
            openEmail(
    ...
    private fun initViewModel() {
        ...
        val repo =
            SpeedHistoryRepositoryImpl(AppDatabase.getInstance(requireContext()).speedHistoryDao())
        val factory = SettingsViewModelFactory(
            DeleteSpeedHistoryUseCase(repo),
            GetAutoSaveUseCase(dataStore),
            SetAutoSaveUseCase(dataStore),
            ObserveSpeedUnitUseCase(dataStore),
            SetSpeedUnitUseCase(dataStore), GetSpeedAlertEnabledUseCase(dataStore),
            SetSpeedAlertEnabledUseCase(dataStore),
            GetSpeedLimitUseCase(dataStore),
            SetSpeedLimitUseCase(dataStore)
    ...
    private fun setupListeners(view: View) {
        ...
        speedUnitValue.addPressEffect { toggleUnitLayout() }
        speedLimitValueTv.addPressEffect { viewModel.toggleLimitLayout() }

        view.findViewById<ImageButton>(R.id.minus_image).addPressEffect { viewModel.updateSpeedLimit(viewModel.speedLimitDisplay.value - 1) }
        view.findViewById<ImageButton>(R.id.plus_image).addPressEffect { viewModel.updateSpeedLimit(viewModel.speedLimitDisplay.value + 1) }
        ...
        clearHistoryTv.addPressEffect { showClearHistoryDialog() }
    }
    ...
    private fun showClearHistoryDialog() {
        Dialog(requireContext()).apply {
            ...
            findViewById<View>(R.id.cancel_tv).addPressEffect { dismiss() }
            findViewById<View>(R.id.ok_tv).addPressEffect { viewModel.clearAllHistory(); dismiss() }
            show()
        }
    }
